...
module cap()
{
    ...
    linear_extrude(height = cap_height) ring(inner, outer);

    threading_height = cap_height - cap_pitch;

    translate(v = [ 0, 0, cap_height / 2 ])
        thread_helix(d = (inner - thread_depth + TINY) * 2, turns = threading_height / cap_pitch / 6, pitch = cap_pitch,
                     starts = 6, thread_depth = thread_depth, internal = true, lead_in = 2);
}

module ledge()
{
    rotate_extrude() translate(v = [ main_tube_radius, 0 ]) polygon(points = [
        [ 0, 0 ],
...
module feet()
{
    ...
    for (k = [0:n_feet - 1])
    {
        rotate([ 0, 0, k * theta * 2 - theta / 2 ]) linear_extrude(height = feet_height + TINY)
...
module inner_ring()
{
    rotate_extrude() translate(v = [ inner_ring_radius, 0 ]) polygon(points = [
        [ inner_ring_width, 0 ],
...
module ring_support()
{
    linear_extrude(height = inner_ring_height) ring_support_2d();
}

module siphon()
{
    translate(v = [ 0, 0, main_tube_height + feet_height - ledge_width + TINY ]) ledge();

    translate(v = [ 0, 0, main_tube_height + feet_height ]) cap();

    translate(v = [ 0, 0, main_tube_height + feet_height - inner_ring_height ]) inner_ring();

    rotate(a = [ 0, 0, 180 / n_feet ]) translate(v = [ 0, 0, main_tube_height + feet_height - inner_ring_height ])
        ring_support();

    translate(v = [ 0, 0, feet_height ]) tube();
...
module untrigger_inner()
{
    ...
    difference()
    {
        linear_extrude(height = feet_height + untrigger_guide_height + untrigger_guide_chamfer) untrigger_inner_2d();

        translate(v = [ main_tube_radius - (untrigger_tube_outer_radius + untrigger_wall), 0, 0 ]) untrigger_tube();
    }
}
...
module untrigger_middle_single_chamfer()
{
    ...
    rotate([ -180 / n_feet / 2, 90, 0 ]) linear_extrude(main_tube_radius + main_tube_width + TINY) polygon([
        [ -TINY, TINY ],
        [ -TINY, -chamfer - TINY ],
        [ chamfer + TINY, TINY ],
    ]);
...
module untrigger_middle_chamfer()
{
    translate([ 0, 0, feet_height ])
    {
        ...
        mirror([ 0, 1, 0 ]) untrigger_middle_single_chamfer();
    }
}
...
module untrigger_middle()
{
    ...
    rotate([ 0, 0, -theta / 2 ]) difference()
    {
        linear_extrude(height = feet_height) intersection()
        {
            ring(inner = inner, outer = outer);
            arc(r = outer, theta = theta);
        };

        rotate([ 0, 0, theta / 2 ]) untrigger_middle_chamfer();
    }
}
...
module untrigger_cavity_2d()
{
    ...
    translate([ outer + untrigger_outer_cavity_radius, 0, 0 ]) circle(r = untrigger_outer_cavity_radius);
}
...
module untrigger_outer_2d()
{
    ...
    difference()
    {
        hull()
        {
            rotate([ 0, 0, -theta / 2 ]) arc_ring(inner = inner, outer = outer, theta = theta);

            translate([ outer + untrigger_outer_cavity_radius, 0, 0 ])
                circle(r = untrigger_outer_cavity_radius + untrigger_wall);
        }

        rotate([ 0, 0, -theta / 2 ]) arc_ring(inner = TINY, outer = inner, theta = theta);
    }
}
...
module untrigger_outer()
{
    ...
    difference()
    {
        ...
        linear_extrude(height = untrigger_height) untrigger_outer_2d();

        translate([ 0, 0, untrigger_wall ]) linear_extrude(height = untrigger_height) untrigger_cavity_2d();
    }
}
...
module untrigger_communication()
{
    ...
    translate([ 0, 0, wall ]) linear_extrude(height = gallery_height) hull(){
        union(){translate([ main_tube_radius - (untrigger_tube_outer_radius + untrigger_wall), 0 ]) circle(r = inner);
...
{
    chamfer = 0.6;
    translate([ 0, 0, untrigger_height ]) rotate_extrude() translate([ main_tube_radius + main_tube_width, 0 ])
        polygon([
            [ -TINY, TINY ],
            [ -TINY, -chamfer - TINY ],
            [ chamfer + TINY, TINY ],
...
rotate([ 0, 0, 180 * (1 + 1 / n_feet / 2) ]) color(c = "red") untrigger();
