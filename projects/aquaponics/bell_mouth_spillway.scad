...
module perforations()
{
    ...
    for (i = [0:lines - 1])
    {
        ...
        translate([ 0, 0, start_h + i * (h + o) ]) linear_extrude(h) for (j = [0:n_eff - 1])
        {
            rotate(j * period) rotate(oa_eff) rotate([ 0, 0, delta / 2 ])
...
module envelope()
{
    ...
    rotate_extrude(180) envelope_sketch();

    rotate([ 90, 0, 0 ]) linear_extrude(outlet_diameter / 2 / 1.4) union()
    {
        envelope_sketch();

        mirror([ 1, 0, 0 ]) envelope_sketch();
    }
}
...
module back_wall()
{
    rotate([ 90, 0, 0 ]) translate([ 0, 0, TINY ]) linear_extrude(wall)
    {
        back_wall_half();

        mirror([ 1, 0 ]) back_wall_half();
...
module box()
{
    ...
    translate([ 0, 0, -box_height / 2 ]) difference()
    {
...
module main()
{
    ...
    difference()
    {
        ...
        rotate([ 90, 0, 0 ]) cylinder(4 * wall, d = outlet_diameter, center = true);
    }
}
...
module pipe()
{
    ...
    rotate([ 0, 90, 0 ]) rotate_extrude() pipe_mouth_sketch();

    translate([ h, 0, 0 ])
    {
        translate([ TINY, 0, 0 ]) rotate([ 0, -90, 0 ]) linear_extrude(h) pipe_sketch(outlet_wall);

        translate([ r, 0, -v - r + TINY ]) linear_extrude(v) pipe_sketch(outlet_wall);

        rotate([ 90, 0, 0 ]) translate([ 0, -r ]) rotate_extrude(90) translate([ r, 0 ]) pipe_sketch(outlet_wall);
    }
}
...
module pipe_adaptor()
{
    ...
    linear_extrude(12) ring(ir, ir + w);
}
...
module half_circle(r)
{
    ...
    {
        circle(r);
        translate([ -(r + TINY / 2), 0 ]) square([ 2 * r + TINY, r + TINY ]);
    }
}
        ...
        for (i = [0:ny])
        {
            ...
            translate([ 0, y ]) union()
            {
                for (j = [-nx:nx])
                {
                    ...
                    translate([ x, 0 ]) circle(r = lid_holes_radius);
                }
            }
        }
    }

    linear_extrude(lid_snap_height) arc_ring(ir - wall - 0.5, ir - 0.5, 180);
}
...
module tube()
{
    or = 9.8 / 2;
    ir = or -1.5;
    linear_extrude(20, scale = 0.9) ring(ir, or);
    translate([ 0, 0, -20 ]) linear_extrude(20) ring(ir, 16 / 2);
}
...
module contact_sketch()
{
    ...
    polygon([
        ...
        [ tube_or, exhaust_insert + ring ],
        [ or, exhaust_insert + ring ],
        [ or, exhaust_insert + ring + tube_insert ],
        [ ir, exhaust_insert + ring + tube_insert ],
    ]);
}
...
module bend()
{
    ...
    rotate([ 90, 0, 0 ]) rotate_extrude(90) translate([ 10, 0 ]) ring(tube_ir, tube_or);
}
