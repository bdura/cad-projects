...
module perforations() {
  ...
  for (i = [0:lines - 1]) {
    ...
    translate([0, 0, wall + o + i * (h + o)])
      linear_extrude(h)for (j = [0:n_eff - 1]) {
...
module main_wall_sketch() {
  difference() {
    ...
    translate([0, -radius]) square([3 * radius, 2 * radius], center=true);
  }

  filleted_wall();

  mirror([1, 0]) filleted_wall();
}

module main_wall() {
  difference() {
    linear_extrude(height) main_wall_sketch();
...
module back_wall() {
  rotate([-90, 0, 0]) rotate([0, 0, -90]) translate([0, 0, -wall / 2])
        linear_extrude(wall) translate([height / 2, 0]) difference() {
              polygon(
                [
              ...
              square(
                [height - 2 * (inner_anchor), 2 * (radius - inner_anchor)],
                center=true
              );
            }
}
